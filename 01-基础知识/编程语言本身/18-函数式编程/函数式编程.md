...
在`Arrays.sort()`中直接传入了静态方法`cmp`的引用，用`Main::cmp`表示。
...
因为实例方法有一个==隐含的`this`参数==，`String`类的`compareTo()`方法在实际调用的时候，第一个隐含参数总是传入`this`，相当于静态方法：
...
把==数组==变成`Stream`使用`Arrays.stream()`方法。
...
因为Java的范型不支持基本类型，所以我们无法用`Stream`这样的类型，会发生编译错误。
为了保存`int`，只能使用`String`，但这样会产生频繁的装箱、拆箱操作。
